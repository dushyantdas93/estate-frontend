...
const Slider = ({ images , setIsSlider }) => {
  ...
  const changeSlider = (direction) => {
    if (direction == "left") { //imageIndex%images.length --> circular iteration
      if (imageIndex === 0) {
      ...
      <div className="w-full h-[350px] flex gap-[20px] " id="slider">
        {imageIndex !== null && (
          <div
            ...
            id="fullslider"
          >
            <div className="flex-1 text-white  text-8xl font-bold" id="arrow" onClick={()=>changeSlider("left")}>
              <IoIosArrowBack />
            </div>
        ...
        <div className="w-[75%]" id="bigimage">
          <img
            src={images[0]}
            className="h-full w-full cursor-pointer object-cover rounded-sm" onClick={()=>setImageIndex(0)}
          />
        </div>
